...
```bash
trunks sandboxes providers doctor local --json
trunks sandboxes providers test local --json
trunks sandboxes providers benchmark local --json
trunks sandboxes providers orphans daytona --json
trunks sandboxes providers cleanup daytona --dry-run --json
scripts/verify-sandbox-provider.sh daytona
```
...
`scripts/verify-sandbox-provider.sh <provider>` checks provider discovery and metadata without creating a sandbox. Add `--live` to run the provider contract and create real provider resources:

```bash
scripts/verify-sandbox-provider.sh daytona --live
```
...
```bash
trunks sandboxes providers test docker --json
...
trunks sandboxes providers add daytona-main \
  --type daytona \
  --secret api_key=...
trunks sandboxes providers show daytona --json
...
trunks sandboxes providers orphans primeintellect --json
trunks sandboxes providers cleanup primeintellect --dry-run --json
```

Provider profiles are stored in the Trunks repository metadata and masked in CLI output. Environment variables such as `TRUNKS_DAYTONA_API_KEY`, `TRUNKS_DIGITALOCEAN_API_TOKEN`, and `TRUNKS_PRIMEINTELLECT_API_KEY` are still accepted for ephemeral local testing or CI overrides.
